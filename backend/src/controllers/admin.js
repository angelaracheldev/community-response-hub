...
// new changes from above

// const db = require('../repositories/db.repository'); // OR your actual db path
...
exports.getComplaintDetails = async (req, res) => {
  try {
    ...
    console.log('Complaint ID received:', complaintId);
    console.log('Params:', req.params);

    console.log('Complaint ID received:', complaintId);
    // Complaint + Category
    // const complaintResult = await db.query(
    //   `SELECT c.complaint_id, c.reference_id, c.reported_by, c.category_id,
    //           cc.category_name, cc.description AS category_description,
    //           c.title, c.description, c.location_text,
    ...
    const complaintResult = await db.query(
  `SELECT c.complaint_id, c.reference_id, c.reported_by,
...
console.log('Rows found:', complaintResult.rowCount);
console.log('Rows:', complaintResult.rows);
    ...
    // Assignments
    const assignmentsResult = await db.query(
      `SELECT ca.assignment_id, ca.complaint_id, ca.assigned_to,
              ca.assigned_by, ca.is_active, ca.assigned_at,
    ...
    // Media
    const mediaResult = await db.query(
      `SELECT media_id, complaint_id, uploaded_by,
              media_url, media_type, uploaded_at
       FROM complaint_media
       WHERE complaint_id = $1
       ORDER BY uploaded_at ASC`,
      [complaintId]
    ...
    // Activity logs
    const logsResult = await db.query(
      `SELECT al.activity_log_id, al.complaint_id,
              al.performed_by, al.action_type,
    ...
    console.error('Failed to fetch complaint details (admin):', error);
