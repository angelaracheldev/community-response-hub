...
// router.get('/complaints/:id/details', authMiddleware, requireRole('admin'), async (req, res) => {
//   try {
...
// // 1ST CHECK
// // This file defines admin-specific routes, such as fetching detailed complaint information for administrative review.
// const router = require('express').Router();
// // const db = require('../db');
...
//                 c.title, c.description, c.location_text, c.latitude, c.longitude,
//                 c.status, c.priority_level, c.remarks, c.created_at, c.updated_at
//          FROM complaints c
//          LEFT JOIN complaint_categories cc ON cc.category_id = c.category_id
//          WHERE c.complaint_id = $1`,
//         [id]
...
//       const logsResult = await db.query(
//         `SELECT al.activity_log_id, al.complaint_id, al.performed_by, al.action_type,
