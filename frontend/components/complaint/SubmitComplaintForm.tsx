...
import { FieldErrors, hasFieldErrors, validateComplaintForm } from '../../utils/validation';
...
export type SubmitComplaintFormProps = {
  portal: AppPortal;
  activeNavId: string;
  ...
  successPrimaryRoute: string;
  successSecondaryLabel: string;
  successSecondaryRoute: string;
};
...
export function SubmitComplaintForm({
  ...
  const pickEvidence = async () => {
    ...
    if (rejected.length) {
      Alert.alert('Unsupported file type', 'Only JPG, JPEG, PNG, MP4, and MOV files are allowed.');
    }
  ...
  if (requireVerification && verificationLoading) {
    return (
      <PageShell portal={portal} activeNavId={activeNavId} pageTitle="Add Complaint" scrollEnabled={false}>
      ...
      <PageShell portal={portal} activeNavId={activeNavId} pageTitle="Add Complaint" scrollEnabled={false}>
        <View style={styles.blockedContainer}>
          ...
          <Text style={styles.blockedText}>
            Your ID is being reviewed by an admin. You cannot submit complaints until your residency
            verification is approved.
          </Text>
          <TouchableOpacity style={styles.primaryBtn} onPress={() => router.replace(backRoute)}>
        ...
        {step < 3 && (
          <TouchableOpacity onPress={handleBack} style={styles.backLink} disabled={isSubmitting}>
            <Text style={styles.backLinkText}>
              {step === 1 ? backLabel : '← Edit Complaint'}
            </Text>
          </TouchableOpacity>
        )}
        ...
        {step === 3 && submittedComplaintId && (
          <>
            ...
            <TouchableOpacity
              ...
              onPress={() => router.replace(successPrimaryRoute)}
            >
              <Text style={styles.primaryBtnText}>{successPrimaryLabel}</Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={styles.secondaryBtn}
              onPress={() => router.replace(successSecondaryRoute)}
            >
              <Text style={styles.secondaryBtnText}>{successSecondaryLabel}</Text>
            </TouchableOpacity>
          </>
        )}
      </ScrollView>
